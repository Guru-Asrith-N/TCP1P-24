...
After going through the code, you realise that it is a man in the middle attack.    
The entity denotes the people A and B who are communicating and the attacker is C.   
Since the functions are already given you can use the given functions to encrypt and decrypt messages.   
Now since we are the attacker, we exchange our public key with A and B do that the communication goes through the attacker.   
Now A sends a message which we intercept and decrypt.  
We then tamper with the message and change the message to `giv me the flag you damn donut` and encrypt using public key of B.   
We then decrypt the message that we recieve which is the flag.   
